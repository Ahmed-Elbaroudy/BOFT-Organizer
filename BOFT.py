...
def save_config():
    try:
        ...
        messagebox.showerror("Error", f"An unexpected error occurred while saving configuration: {e}")
        return False
...
def load_config():
    ...
    try:
        if os.path.exists(CONFIG_FILE):
            ...
            if not isinstance(config.get("file_categories", {}), dict):
                config["file_categories"] = DEFAULT_CONFIG["file_categories"]
            
            # Validate stats
            if not isinstance(config.get("stats", {}), dict):
                config["stats"] = DEFAULT_CONFIG["stats"]
            
            # Validate custom organization settings
            if not isinstance(config.get("custom_organization", {}), dict):
                config["custom_organization"] = DEFAULT_CONFIG["custom_organization"]
                
            return True
        else:
            # Initialize with default config if no config file exists
        ...
        messagebox.showerror("Error", f"Invalid configuration file. Using default settings.")
        config.update(DEFAULT_CONFIG)
        save_config()
        return False
    except (PermissionError, OSError) as e:
        ...
        messagebox.showerror("Error", f"An unexpected error occurred while loading configuration: {e}")
        config.update(DEFAULT_CONFIG)
        return False
...
def organize_files_by_type(directory, silent=True):
    try:
        ...
        for file in files:
            try:
                ...
                if category:
                    ...
                    if category in stats["files_by_type"]:
                        stats["files_by_type"][category] += 1
                    else:
                        stats["files_by_type"][category] = 1
                    
                    processed_files += 1
                    if not silent:
                        show_progress(f"Processing files... ({processed_files}/{file_count})", 
                                    processed_files / file_count * 0.7 + 0.2)
                        
                    # Log activity
                    log_activity("Organized", file_path, dest_path, category)
        ...
        if not silent:
            hide_progress()
            update_status(f"Organized {processed_files} files in {directory}")
            
    except Exception as e:
        print(f"Critical error in organize_files_by_type: {e}")
        if not silent:
            hide_progress()
...
def start_auto_organize():
    ...
    if auto_organize_folder:
        try:
            ...
            observer.schedule(FileChangeHandler(), auto_organize_folder, recursive=True)
            observer.start()
            
            # Start the auto-organize loop in a separate thread
            threading.Thread(target=auto_organize_loop, daemon=True).start()
            update_status(f"Auto-organize started for: {auto_organize_folder}")
            
        except Exception as e:
            print(f"Error starting auto-organize: {e}")
            messagebox.showerror("Error", f"Failed to start auto-organize: {str(e)}")
            auto_organize_running = False
...
def stop_auto_organize():
    ...
    try:
        ...
        print(f"Error stopping auto-organize: {e}")
        messagebox.showerror("Error", f"Failed to stop auto-organize: {str(e)}")
...
class FileChangeHandler(watchdog.events.FileSystemEventHandler):
    ...
    def on_created(self, event):
        try:
            ...
            # Only process if it's been at least 1 second since last event
            if current_time - self.last_event_time >= 1:
                ...
                if os.path.exists(file_path):
                    ...
                    if category:
                        ...
                        # Log activity
                        log_activity("Auto-Organized", file_path, dest_path, category)
                        update_status(f"Auto-organized: {os.path.basename(file_path)}")
                        
        except Exception as e:
            print(f"Error in file change handler: {e}")
            messagebox.showerror("Error", f"Error processing new file: {str(e)}")
...
def on_closing():
    if config.get("minimize_to_tray", True):
        minimize_to_tray()
        return  # Prevent the window from closing when minimizing to tray
    else:
        exit_program()
        root.quit()  # Ensure the window closes properly
        return "break"
